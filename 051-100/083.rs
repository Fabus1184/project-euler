...
let matrix = file_fetcher::open_str("https://projecteuler.net/project/resources/p083_matrix.txt")
    .unwrap()
    .bytes()
...
println!("{:?}", dijkstra((0, 0),
        ...
        &|x| x == (matrix.len() - 1, matrix.len() - 1))
        .unwrap()
        .iter()
        .map(|(row, col)| matrix[*row][*col]).sum::<i64>()
);
