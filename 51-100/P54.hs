...
data Rank
    ...
    | Straight [Card]
    | Flush [Card]
    | FullHouse ((Card, Card, Card), (Card, Card)) [Card]
...
rank :: Hand -> Rank
rank (Hand cards) = head $ catMaybes ranks
  where
    ranks =
        [ royalFlush
        , straightFlush
    ...
    groups n =
        filter ((== n) . length)
            . groupBy (\(Card v1 _) (Card v2 _) -> v1 == v2)
            . sort
            $ cards
    pairs = map (\[x, y] -> (x, y)) $ groups 2
    triplets = map (\[x, y, z] -> (x, y, z)) $ groups 3
    quadruplets = map (\[w, x, y, z] -> (w, x, y, z)) $ groups 4
    safeLast [] = Nothing
    safeLast xs = Just $ last xs
    toRank2 c (x, y) = c (x, y) (cards \\ [x, y])
    toRank3 c (x, y, z) = c (x, y, z) (cards \\ [x, y, z])
    toRank4 c (w, x, y, z) = c (w, x, y, z) (cards \\ [w, x, y, z])
    isSequence xs = and . zipWith ((==) . succ) (sort xs) $ tail (sort xs)
    royalFlush = (straightFlush >>= (\(StraightFlush xs) -> guard $ value (maximum xs) == Ace)) $> RoyalFlush cards
    straightFlush = straight *> flush $> StraightFlush cards
    fourOfAKind = toRank4 FourOfAKind <$> safeLast quadruplets
    fullHouse = do
        (w, x, y) <- safeLast triplets
        (z, v) <-
            safeLast $
                filter (\(a, b) -> all (`notElem` [w, x, y]) [a, b]) pairs
        pure $ FullHouse ((w, x, y), (z, v)) (cards \\ [w, x, y, z, v])
    flush = guard (all ((== color (head cards)) . color) cards) $> Flush cards
    straight = guard (isSequence . map value $ cards) $> Straight cards
    threeOfAKind = toRank3 ThreeOfAKind <$> safeLast triplets
    twoPair = do
        (x, y) <- safeLast pairs
        (z, v) <- safeLast $ init pairs
        pure $ TwoPair (x, y) (cards \\ [x, y, z, v])
    pair = toRank2 Pair <$> safeLast pairs
    highCard = Just $ HighCard (maximum cards) (cards \\ [maximum cards])

player1Wins :: Hand -> Hand -> Bool
player1Wins h1 h2 = rank h1 > rank h2
...
pokerHands =
    toInteger
        . length
        . filter id
        . map (uncurry player1Wins . bimap read read)
        $ hands
